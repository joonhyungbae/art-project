...
1. `art-paper/references/policy_anchor_table.md` — Nature anchor (#108 policy-anchor track, verbatim quotes per 16 fields)
2. `art-paper/references/venue_disclosure_policies.md` — Nature venue entry (v3.2 venue track, summary form)

**Provenance**: extracted from the Nature Portfolio AI editorial policy page (`https://www.nature.com/nature-portfolio/editorial-policies/ai`), wayback snapshot `nature:wayback=20260513075542` (sha256 `cf691cba…`). Section structure: `§AI authorship`, `§Generative AI images`, `§AI use by peer reviewers`, `§Editorial use`. art-paper author-side matrix covers `§AI authorship` + `§Generative AI images` only.
