...
**Used by**: `compliance_agent` (Task 8), `art-pipeline/SKILL.md` Stage 2.5 / 4.5.
...
|---|---|---|
| `[MATERIAL GAP]` | `gaps[].reason`, `principle_evidence[]`, `evidence[]` | The item cannot be verified because the underlying material (passport field, manuscript section, supplementary file) is missing. Apply [Anti-Leakage Protocol](../art-paper/references/anti_leakage_protocol.md) — do not hallucinate. |
...
On any successful override, the agent generates `disclosure_addendum` text and the orchestrator **auto-injects** it into the manuscript's AI disclosure section. The addendum is non-removable — this is the concrete form of the *assistive, not deceptive* design principle in [`POSITIONING.md`](../POSITIONING.md).
...
`material_passport.compliance_history[]` never overwrites. On retry-pass, the previous fail entry is preserved with its own timestamp. The AI Self-Reflection Report at Stage 6 cites the full history to demonstrate how the correction was made — this is RAISE transparency applied to art-paper itself.

## Boundary: non-pipeline invocation

When invoked from `art-paper full` (primary research standalone), no pipeline state machine is available. Compliance_agent:
