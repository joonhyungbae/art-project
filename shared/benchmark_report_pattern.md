...
art-paper inherits this risk the moment a user publishes a "art-paper beats manual" claim. This
pattern defines a mandatory schema for anyone publishing a art-paper benchmark comparison.
Reports that don't satisfy the schema aren't "art-paper benchmarks" — they're anecdotes.
...
- **`author_independence`** (enum: author-conducted | author-blinded | third-party-conducted):
  ...
  flags the downward-bias risk. Authors know what a good art-paper output looks like; their
  human effort is calibrated against that prior. `author-blinded` (authors worked without
  seeing art-paper outputs first) is better. `third-party-conducted` eliminates experimenter
  bias entirely.
...
- **`tools_allowed`** (array of strings): "No tools" vs "ChatGPT allowed" is not the same
  comparison as art-paper. If the human baseline used no AI assistance while art-paper ran full
...
- **`data_access_level_declared`** (enum: raw | redacted | verified_only): If art-paper ran
...
- **`skills_used`** (array, minItems 1): A benchmark running `art-pipeline`
  full-mode is not comparable to one running only `art-inquiry`. Both are art-paper;
...
output came from art-paper versus human baseline) is the minimum credible standard for any
...
- **Omitting sample size.** The field is required. There is no valid art-paper benchmark report
...
- **Claiming art-paper advantage without disclosing cost_usd.** The field is required. A claim
  that "art-paper completed this in 4 hours vs 40 human hours" without a dollar figure omits
...
Per-skill benchmark templates (a `art-inquiry`-specific template, an
`art-reviewer`-specific template) are likely as v3.4 adds domain-specific
