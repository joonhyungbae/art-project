...
**This is entirely optional.** All art-paper skills work with Claude Opus 4.7 alone. Cross-model verification is an additional layer for users who want higher confidence in integrity checks, devil's advocate challenges, and review judgments.
...
|-------|--------|----------|----------|
| Claude Opus 4.7 | `claude-opus-4-7` | Anthropic | Primary model (default for all art-paper skills) |
...
Using two non-Anthropic models as primary+verifier is possible but not tested with art-paper prompts.

## Setup Guide

### Prerequisites

You need API keys from at least one additional provider. art-paper itself runs inside Claude Code, so Claude is always available as the primary model.
...
```bash
# Optional: Cross-model verification for art-paper
...
export CRS_CROSS_MODEL="gpt-5.4-pro"
```
...
```
Check if cross-model verification is available for art-paper
```
...
unset CRS_CROSS_MODEL
```

## How It Works in Each Skill

### Integrity Verification (art-pipeline, Stage 2.5 / 4.5)

**When `CRS_CROSS_MODEL` is set:**
...
**When `CRS_CROSS_MODEL` is not set:**
- Standard single-model verification (unchanged from v2.7+)

**Implementation for agents:**

When the integrity_verification_agent detects `CRS_CROSS_MODEL` in the environment, it should:
...
### Devil's Advocate (art-inquiry + art-reviewer)
...
### Peer Review (art-reviewer) — Future
...
**Planned behavior when `CRS_CROSS_MODEL` is set:**
...
curl -s "https://generativelanguage.googleapis.com/v1beta/models/${CRS_CROSS_MODEL}:generateContent?key=$GOOGLE_AI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
if [ -n "$CRS_CROSS_MODEL" ]; then
  case "$CRS_CROSS_MODEL" in
    gpt-5.4*) 
      [ -n "$OPENAI_API_KEY" ] && echo "CROSS_MODEL_AVAILABLE=openai" \
        || echo "WARNING: CRS_CROSS_MODEL=$CRS_CROSS_MODEL but OPENAI_API_KEY is not set" ;;
    gemini*) 
      [ -n "$GOOGLE_AI_API_KEY" ] && echo "CROSS_MODEL_AVAILABLE=google" \
        || echo "WARNING: CRS_CROSS_MODEL=$CRS_CROSS_MODEL but GOOGLE_AI_API_KEY is not set" ;;
    *) echo "WARNING: CRS_CROSS_MODEL=$CRS_CROSS_MODEL is not a supported model. Supported: gpt-5.4, gpt-5.4-pro, gemini-3.1-pro-preview"
...
If `CRS_CROSS_MODEL` is set but the corresponding API key is missing or the model name is unsupported, the agent should warn the user and proceed with single-model verification.
