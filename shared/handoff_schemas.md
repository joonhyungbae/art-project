...
## Schema 1: RQ Brief (art-inquiry -> art-paper)

**Producer**: `art-inquiry/research_question_agent` | `art-inquiry/socratic_mentor_agent`
**Consumer**: `art-inquiry/research_architect_agent` | `art-paper/intake_agent`
...
## Schema 2: Bibliography (art-inquiry -> art-paper)

**Producer**: `art-inquiry/bibliography_agent`
**Consumer**: `art-inquiry/synthesis_agent` | `art-inquiry/source_verification_agent` | `art-paper/literature_strategist_agent`
...
**Coverage Assessment**: Strong coverage of English-language literature. Gap: limited grey literature beyond English-language venues.
...
## Schema 3: Synthesis Report (art-inquiry -> art-paper)

**Producer**: `art-inquiry/synthesis_agent`
**Consumer**: `art-inquiry/report_compiler_agent` | `art-paper/argument_builder_agent`
...
## Schema 4: Paper Draft (art-paper -> integrity/reviewer)

**Producer**: `art-paper/draft_writer_agent`
**Consumer**: `art-pipeline/integrity_verification_agent` | `art-reviewer/*`
...
| `abstract` | string | Abstract text (English; art-paper v0.1 default — bilingual machinery inherited but not v0.1 default) |
| `authors` | list[Author] | Author information with CRediT roles |
| `keywords` | list[string] | Keywords (4-6; 3-5 for Pattern 2) |
...
**Producer**: `art-pipeline/integrity_verification_agent`
**Consumer**: `art-pipeline/pipeline_orchestrator_agent` | `art-paper/draft_writer_agent` (for revision)
...
Dimensions match the 7 universal review dimensions from `art-reviewer/references/review_criteria_framework.md` plus an overall score:
...
## Schema 6: Review Report (art-reviewer -> pipeline)

**Producer**: `art-reviewer/editorial_synthesizer_agent`
**Consumer**: `art-pipeline/pipeline_orchestrator_agent` | `art-paper/draft_writer_agent`
...
## Schema 7: Revision Roadmap (reviewer -> art-paper revision)

**Producer**: `art-reviewer/editorial_synthesizer_agent`
**Consumer**: `art-paper/draft_writer_agent` | `art-pipeline/pipeline_orchestrator_agent`
...
## Schema 8: Response to Reviewers (art-paper revision -> reviewer re-review)

**Producer**: `art-paper/draft_writer_agent` (revision mode)
**Consumer**: `art-reviewer/editorial_synthesizer_agent` (re-review)
...
| Field | Type | Description |
|-------|------|-------------|
| `origin_skill` | string | Which skill produced this artifact (e.g., `art-inquiry`, `art-paper`) |
...
| `reset_boundary` | list[object] | Append-only ledger. Two entry kinds: `boundary` (recorded at FULL checkpoints when `CRS_PASSPORT_RESET=1`) and `resume` (recorded when `resume_from_passport` consumes a boundary). Added v3.6.3+. Entry shape: [`shared/contracts/passport/reset_ledger_entry.schema.json`](contracts/passport/reset_ledger_entry.schema.json). See [`art-pipeline/references/passport_as_reset_boundary.md`](../art-pipeline/references/passport_as_reset_boundary.md). |
| `literature_corpus` | list[object] | Optional append-friendly literature corpus. Each entry conforms to [`shared/contracts/passport/literature_corpus_entry.schema.json`](contracts/passport/literature_corpus_entry.schema.json). Produced by user-written adapters (see [`art-pipeline/references/adapters/overview.md`](../art-pipeline/references/adapters/overview.md)); art-paper does not produce these entries itself. Added v3.6.4+. |
...
| `slr_lineage` | boolean | Run-level provenance flag set by `pipeline_orchestrator_agent` at the Stage 1 → Stage 2 handoff. `true` iff any stage in this run history was produced by `art-inquiry` in systematic-review mode. Consumed by `disclosure` mode renderer (`--policy-anchor=prisma-trAIce` track gate per `policy_anchor_disclosure_protocol.md` §3.1). Absence = `false` = cold-start path (renderer requires explicit `mode=` per §4.3 G2 invariant fallback rule). Added v3.7.4+. See [Run-level lineage signal (v3.7.4)](#run-level-lineage-signal-v374) below. |
...
When `CRS_PASSPORT_RESET=1`, Schema 9 gains an append-only `reset_boundary[]` ledger with two entry kinds: `boundary` (recorded at FULL checkpoints) and `resume` (recorded when a boundary is consumed):
...
See [`art-pipeline/references/passport_as_reset_boundary.md`](../art-pipeline/references/passport_as_reset_boundary.md) for the full protocol.
...
art-paper does not produce these entries. User-written adapters read their own corpus source (Zotero, Obsidian, folder, Notion, etc.) and emit a passport with `literature_corpus[]` populated. Three reference adapters ship with v3.6.4 under [`scripts/adapters/`](../scripts/adapters/).

Consumer integration ships in v3.6.5: `bibliography_agent` (art-inquiry, Phase 1) and `literature_strategist_agent` (art-paper, Phase 1) read `literature_corpus[]` via the corpus-first, search-fills-gap flow. See [`art-pipeline/references/literature_corpus_consumers.md`](../art-pipeline/references/literature_corpus_consumers.md) for the full consumer protocol, the four Iron Rules, and per-consumer reading instructions.

See [`art-pipeline/references/adapters/overview.md`](../art-pipeline/references/adapters/overview.md) for the adapter contract.
...
**This mirrors the v3.6.3 `reset_boundary[]` append-only pattern**: history preserved, freshness computed by ledger scan. Deletion or reordering is forbidden; lint at `scripts/check_audit_artifact_consistency.py` enforces the invariant family at [`ref/academic-research-skills/docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md`](../ref/academic-research-skills/docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md) §3.7.

For the orchestrator-side gate procedure (Path A latest-by-`verified_at` selection, Path B proposal merge after Layer 2 + Layer 3 verification), the canonical contract is [`ref/academic-research-skills/docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md`](../ref/academic-research-skills/docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md) §5.6 (Path A/B fall-through with the §5.6 A1.5 superseding-proposal preflight) plus §5.2 (eleven Layer 2 + Layer 3 gating checks). Implementation lands as a subsection of `art-pipeline/agents/pipeline_orchestrator_agent.md` (Phase 6.6 deliverable). For the resume-time re-verification semantics, see [`art-pipeline/references/passport_as_reset_boundary.md`](../art-pipeline/references/passport_as_reset_boundary.md).
...
- Consumer: `disclosure` mode renderer reads it as `RendererInput.slr_lineage` to dispatch `--policy-anchor=prisma-trAIce` per the §4.3 G2 invariant track gate documented in [`art-paper/references/policy_anchor_disclosure_protocol.md`](../art-paper/references/policy_anchor_disclosure_protocol.md) §3.1.
...
Spec: [`ref/academic-research-skills/docs/design/2026-05-15-issue-111-slr-lineage-emission-design.md`](../ref/academic-research-skills/docs/design/2026-05-15-issue-111-slr-lineage-emission-design.md). Conformance test: `scripts/test_slr_lineage_emission.py`.

### Claim-Faithfulness Audit Aggregates (v3.8)

v3.8 introduces six passport aggregates around the L3 (claim-faithfulness) audit. They ride in their own arrays on the audit run record rather than under a root `material_passport` schema (per v3.8 spec §8 explicit scope), and only the four audit-output aggregates are gated on `CRS_CLAIM_AUDIT=1`; the writer-side manifest aggregate and the sampling-summary record are independent.

**Writer-side (pre-commitment baseline — NOT gated on CRS_CLAIM_AUDIT):**
...
**Audit output (gated on CRS_CLAIM_AUDIT=1):**
...
Cross-field invariants (INV-1..INV-18 / M-INV-1..M-INV-4 / U-INV-1..U-INV-4 / D-INV-1..D-INV-4 / CV-INV-1..CV-INV-4 / S-INV-1..S-INV-4) are lint-enforced by `scripts/check_claim_audit_consistency.py` because the conditional matrix relating judgment / audit_status / defect_stage / ref_retrieval_method exceeds what JSON Schema can express. Audit-side producer: `claim_ref_alignment_audit_agent` (`art-pipeline/agents/`). Consumer: `formatter_agent` REFUSE rules 6-10 (see v3.8 spec §5 mode flag rationale). Default OFF for v3.8.0 — ramp-on plan deferred to post-calibration evidence.

Spec: [`ref/academic-research-skills/docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md`](../ref/academic-research-skills/docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md) + decision doc [`2026-05-15-issue-103-claim-alignment-audit-decision.md`](../ref/academic-research-skills/docs/design/2026-05-15-issue-103-claim-alignment-audit-decision.md) (D1-D6 settled).

---

## Schema 10: Style Profile (intake -> draft_writer / report_compiler)

**Producer**: `art-paper/agents/intake_agent` (Step 10)
**Consumer**: `art-paper/agents/draft_writer_agent`, `art-inquiry/agents/report_compiler_agent`
**Carried by**: `art-pipeline` Material Passport (optional field)
...
**Producer**: art-reviewer (re-review mode)
**Consumer**: art-paper (revision mode, if further revision needed), pipeline orchestrator
...
This is a declarative truth-in-advertising signal. All current art-paper skills are `open-ended` because art-paper targets humanities/QA/policy work, not benchmark tasks. When adding a new skill, do not invent a third value; if the skill genuinely spans both, split it into two skills.
...
- `benchmark_report.schema.json` + [`benchmark_report_pattern.md`](benchmark_report_pattern.md) — schema for publishing art-paper benchmark comparisons with required human baseline + independence fields.
