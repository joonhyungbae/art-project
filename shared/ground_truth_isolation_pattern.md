...
Two published cases make this concrete and directly inform the ARS design (inherited by art-paper).
...
art-paper's human-in-the-loop pipeline already enforces the spirit of this
...
Every artifact in art-paper belongs to one of three layers, and the direction of
...
downstream as verified fact without explicit gate passage. The `art-inquiry`
skill operates here.
...
No art-paper skill operates on layer 3 inputs and produces layer 1 or 2 outputs.
...
The isolation pattern is already instantiated across the art-paper codebase through
a set of narrowly scoped protocol files. This section is a navigational map —
not a duplication of their contents.

| Mechanism | Where it lives |
|---|---|
| Source verification (S2 API) | `art-inquiry/references/semantic_scholar_api_protocol.md` |
| Anti-leakage protocol | `art-paper/references/anti_leakage_protocol.md` |
| Integrity gates (Stage 2.5/4.5) + 7-mode failure checklist | `art-pipeline/references/ai_research_failure_modes.md` |
| Reviewer calibration mode (FNR/FPR with private gold set) | `art-reviewer/references/calibration_mode_protocol.md` |
...
**Not a runtime permission system.** Nothing in art-paper enforces isolation at
...
**Not a benchmark protocol.** All current art-paper skills are `task_type:
open-ended` because art-paper targets humanities research, higher-education quality
...
infrastructure for running art-paper outputs through automatic scoring against a
