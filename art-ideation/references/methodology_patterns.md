...
> **art-paper status — NON-DEFAULT (empirical apparatus).** The default for art-paper is **practice-based research** (inquiry-through-making; the artwork is the primary contribution) — see `research_architect_agent.md`, `shared/references/art_research_evidence_model.md`, and the art-paper structure patterns in `shared/references/art_paper_structure_patterns.md`. The empirical design templates below (systematic review, experiment, etc.) apply ONLY to the empirical sub-question of an art-science hybrid (Pattern 5). Never force IMRaD onto a practice-based contribution.

## Purpose
Ready-to-use methodology templates for empirical research designs. Used by the research_architect_agent ONLY for an art-science hybrid sub-question; the default is practice-based / practice-led inquiry.
