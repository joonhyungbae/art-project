...
=== Layer 5: SIGNIFICANCE & CONTRIBUTION (conclusion) ===
     ...
     +-> Compile all [INSIGHT]s into Research Plan Summary
         Can directly hand off to art-paper (plan mode)
```
...
When `CRS_SOCRATIC_READING_PROBE=1`, the Mentor runs a one-time honesty probe at the Layer 2 → Layer 3 transition, but only for goal-oriented sessions where the user has already cited a specific paper.
