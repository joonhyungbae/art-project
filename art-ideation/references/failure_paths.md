...
This document lists all failure scenarios that may be encountered across all modes of the art-inquiry skill, along with their detection conditions, user notification messages, handling steps, and recovery paths. The purpose is to ensure every failure scenario has a clear handling strategy, preventing users from reaching a dead end.
...
| F8 | Only non-English-language literature available | full, lit-review | Medium | Note language gap; expand search |
...
### F8: Only Non-English-Language Literature Available

**Affected Modes**: `full` (Phase 2), `lit-review`
**Severity**: Medium

**Trigger Conditions**:
- English academic/art database searches yield empty or very few results
- The work or its lineage is strongly localized (e.g., a regional art scene, a non-English-language artist or movement, locally exhibited precedents with little English-language coverage)

**User Notification Message**:
> English-language literature on this topic is limited and the strongest precedents may sit in a non-English-language scene. art-paper v0.1 paper output is English-only, so I will keep the report in English while expanding the search to surface comparable international cases. The localized precedents will be cited where load-bearing, with language explicitly annotated.

**Handling Steps**:
1. Expand the English-language search (synonyms, citation chains from named precedents, festival/museum archives)
2. Cite localized precedents where load-bearing; annotate the language in the report
3. If the user needs international publication → suggest finding comparable international cases and noting the lineage gap explicitly

**Recovery Paths**:
- Expanded English-language search yields enough → continue workflow
- Localized lineage is essential → cite it explicitly with language annotation + note the gap as part of the contribution
