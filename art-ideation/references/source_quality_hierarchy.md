...
> **art-paper status — NON-DEFAULT (superseded for the art-research genre).** For practice-based art research, the canonical evidence model is `shared/references/art_research_evidence_model.md`: **the artwork is primary evidence**, and claims are triangulated across work / process / exhibition / lineage / reflection — NOT ranked by study design. Ranking meta-analysis > RCT > case report is a category error for an art paper. This 7-level pyramid remains available ONLY for the empirical sub-question of an art-science hybrid (Pattern 5) or lit-review mode. `source_verification_agent` and `bibliography_agent` default to the art-research evidence model.

## Purpose
Systematic framework for grading evidence quality. Used ONLY for an explicit empirical sub-question (art-science hybrid / Pattern 5); the default is the art-research evidence model above.
