...
**Architectural friction (v0.2 — HARD, was UNENFORCED at v0.1).** Backed by a real append-only log at `~/.art-project/rehearsal-log.jsonl` (one JSON object per rehearsal invocation, schema `{timestamp, brief_codename, brief_hash?, outcome}`). On invocation, the skill reads the log, filters entries matching the artist-supplied brief codename within the last 14 days, and if the count is ≥ 2 fires the warning verbatim: *"You have rehearsed this concept multiple times. Consider showing it to an external reader before further rehearsal — the marginal value of additional rehearsal is low compared to one round of real feedback."* The warning is **consultative, not blocking** — the artist still owns the proceed/abort decision. The v0.2 upgrade is that the count comes from runtime state rather than artist self-report; see [`commands/rehearsal.md`](../commands/rehearsal.md) "Architectural friction (consultative log at v0.2)" for the full protocol, including the graceful fallback when the log is absent or unreadable.
