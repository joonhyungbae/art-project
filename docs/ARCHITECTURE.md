...
|---|---|---|
| 🧑 1 | 1. INQUIRY | Concept & Provocation Brief + Practice-Based Methodology Blueprint |
| 🧑 2 | 2. WRITE | Outline approval before drafting |
| 🧑 3 | 3. JURY REVIEW | Editorial decision (Accept / Minor / Major / Reject) |
...
| 🧑 9 | 5. FINALIZE | Output format selection (acmart LaTeX → PDF canonical / MD / DOCX) |
...
|---|---|---|---|
| ✓ 1 | 2.5 INTEGRITY | 7-mode failure checklist (see §3 for exact taxonomy), re-scoped to artwork / realization claim verification | Integrity Report PASS/FAIL + any SUSPECTED flags |
...
flowchart TD
    Start([User input])
    S1[1. INQUIRY<br/>🧑 art-inquiry<br/>👁 observer]
    S2[2. WRITE<br/>🧑 art-paper<br/>👁 observer]
    G25{{2.5 INTEGRITY<br/>✓ 7-mode checklist<br/>artwork/realization claims<br/>then user ack<br/>— observer SKIPPED}}
    S3[3. JURY REVIEW<br/>🧑 art-reviewer<br/>👁 observer]
    D3{Decision}
    RC[🧑 3→4 Revision<br/>Coaching<br/>max 8 rounds]
    S4[4. REVISE<br/>🧑 art-paper<br/>👁 observer]
    ...
    S5[5. FINALIZE<br/>🧑 format selection<br/>acmart LaTeX → PDF]
...
|---|---|---|---|---|---|
| **1. INQUIRY** | `art-inquiry` v0.1 (full / socratic / lit-review / systematic-review / fact-check / review / quick) | RAW | Concept & Provocation Brief; Practice-Based Methodology Blueprint; Annotated Bibliography (precedent works + theory, S2-verified for the theory/precedent literature); Synthesis Report; INSIGHT Collection. **Search Strategy report includes PRE-SCREENED block (v3.6.5)** when Material Passport carries `literature_corpus[]` | research_question_agent; research_architect_agent; **📚 bibliography_agent (v3.6.5+ corpus reader — corpus-first / search-fills-gap flow)**; source_verification_agent; synthesis_agent; meta_analysis_agent; editor_in_chief_agent; devils_advocate_agent; risk_of_bias_agent; ethics_review_agent; **🟦 socratic_mentor_agent (v3.5.1 reading-check probe layer, opt-in)**; report_compiler_agent; monitoring_agent (13 agents); **👁 collaboration_depth_agent (v3.5.0, advisory)** | 🧑 **Decision-heavy checkpoint:** user confirms concept/provocation brief + practice-based methodology. Machine checks: S2 API Tier-0 verification (Levenshtein ≥ 0.70) on the theory/precedent literature; artwork-as-primary-evidence model applied (triangulate work / process / exhibition / lineage / reflection per `shared/references/art_research_evidence_model.md`); anti-sycophancy on DA (score 1-5, concede only ≥ 4); **corpus-first flow with 4 Iron Rules + F3/F4 provenance reporting (v3.6.5)** when corpus present. 👁 Observer runs post-checkpoint; never blocks |
| **2. WRITE** | `art-paper` v0.1 (full / plan / outline-only / lit-review / revision-coach / abstract-only / citation-check / disclosure / format-convert / revision / **artist-statement** / **work-doc**) | REDACTED | Paper Configuration Record; Outline (default Practice-Based Art Paper structure: context → conceptual framework → the work → realization → reflection, per `shared/references/art_paper_structure_patterns.md`); Argument Map; Draft Text; Abstract; Documentation Figures + Captions; Citation List (ACM Reference Format). **Literature Search Report includes PRE-SCREENED block (v3.6.5)** when Material Passport carries `literature_corpus[]`; merged `final_included` set feeds the Literature Matrix and lineage/gap identification | 12-agent pipeline: intake_agent; **📚 literature_strategist_agent (v3.6.5+ corpus reader — corpus-first / search-fills-gap flow)**; structure_architect_agent; argument_builder_agent; draft_writer_agent; citation_compliance_agent; abstract_agent; peer_reviewer_agent; formatter_agent; socratic_mentor_agent; visualization_agent; revision_coach_agent; **👁 collaboration_depth_agent (v3.5.0, advisory)** | 🧑 **Decision-heavy checkpoint:** outline approved before drafting. Machine checks: anti-leakage protocol (unsupported fill → `[MATERIAL GAP]`); VLM figure verification on documentation images (checklist, max 2 refinements); style calibration vs user voice; Stage 2 parallelization (Phase 1 + visualization after outline); **corpus-first flow with 4 Iron Rules + F3/F4 provenance reporting (v3.6.5)** when corpus present. The bilingual-abstract machinery is inherited but not the v0.1 default — verify SIGGRAPH Asia Art Papers / ACM language requirements against the current CFP. 👁 Observer runs post-checkpoint; never blocks |
| **2.5 INTEGRITY** | `art-pipeline` v0.1 (gate) | VERIFIED_ONLY | Material Passport (Schema 9, required) + `repro_lock` (v3.3.5, declared — populated or `null`); Claim Verification Report re-scoped to **artwork / realization claim verification** (pre-review sampling: 30% of claims, min 10 — per `claim_verification_protocol.md`): reception claims need an observable anchor (named venue/date + observable detail), novelty/precedence and technical-capability claims need an anchor or a hedge; Provenance/Credit Audit (collaboration credit, copyright/exhibition rights) | integrity_verification_agent; state_tracker_agent; pipeline_orchestrator_agent. **👁 collaboration_depth_agent: SKIPPED (MANDATORY gate — observer dilution explicitly prevented)** | ✓ **Integrity gate** + user ack. 7-mode AI failure checklist (Lu 2026, canonical order per `ai_research_failure_modes.md`), gate structure unchanged, verification target re-scoped to artwork/realization: **M1** realization claim passing AI self-review unverified; **M2** hallucinated citation; **M3** hallucinated/inflated reception or exhibition result; **M4** shortcut reliance; **M5** technical artifact reframed as conceptual insight; **M6** methodology/process fabrication; **M7** frame-lock at early pipeline stage. Pre-review claim sampling mode. FAIL → fix + re-verify (max 3 rounds) |
| **3. JURY REVIEW** | `art-reviewer` v0.1 (full / guided / quick / realization-focus / calibration) | VERIFIED_ONLY | **First-round jury package** (per `art-reviewer/SKILL.md`): 5 review reports simulating the SIGGRAPH Asia Art Papers jury (Chair + curator + practitioner-researcher + art-science critic + Devil's Advocate) + Editorial Decision (Accept / Minor / Major / Reject — final acceptance is the venue's, verify against the current CFP) + Revision Roadmap. **v3.6.2 Schema 13 Sprint Contract (`shared/sprint_contract.schema.json`) is required** for `full` and `realization-focus` modes (other modes reserved with pre-v3.6.2 behaviour). | field_analyst_agent (auto-detects art subfield + SIGGRAPH track, configures 3 field-adaptive jurors); eic_agent (Chair); methodology_reviewer_agent (practitioner-researcher / realization); domain_reviewer_agent (curator); perspective_reviewer_agent (art-science critic); devils_advocate_reviewer_agent; **🔒 editorial_synthesizer_agent (v3.6.2 three-step mechanical protocol + forbidden-ops list)** (7 agents); **👁 collaboration_depth_agent (v3.5.0, advisory)** | 🧑 **Decision-heavy checkpoint:** user reviews editorial decision. Machine checks: concession threshold protocol (DA rebuttal scored 1-5, no concede below 4); attack intensity preserved through revisions; cross-model DA critique (optional, `CRS_CROSS_MODEL` env); read-only constraint (no new claims). **v3.6.2 Sprint Contract two-phase protocol**: each juror runs paper-content-blind Phase 1 (commits scoring plan) then paper-visible Phase 2 via `<phase1_output>` data delimiter; synthesizer runs three-step mechanical protocol (build matrix → evaluate with panel-relative quantifier → resolve precedence by severity). Validated by `scripts/check_sprint_contract.py`. 👁 Observer runs post-checkpoint; never blocks |
| **3 → 4 Revision Coaching** | `art-reviewer` (EIC Socratic sub-stage) | VERIFIED_ONLY | Revision strategy dialogue (not an artifact handed forward; feeds Stage 4 revision plan) | eic_agent | 🧑 **Decision-heavy checkpoint:** Socratic dialogue with EIC (max 8 rounds). User may say "just fix it for me" to skip. Source: `two_stage_review_protocol.md` |
| **4. REVISE** | `art-paper` v0.1 (revision / revision-coach) | REDACTED | Point-by-Point Response; Revised Draft; Delta Report (what changed + why) | revision_coach_agent (v3.3 Socratic mode); draft_writer_agent (re-entry); argument_builder_agent (if structural); **👁 collaboration_depth_agent (v3.5.0, advisory)** | 🧑 **Decision-heavy checkpoint:** user confirms changes. Machine checks: score trajectory tracked per rubric dimension (v3.3) — revisions that regress a dimension are flagged. 👁 Observer runs post-checkpoint; never blocks |
| **3'. RE-REVIEW** | `art-reviewer` v0.1 (re-review) | VERIFIED_ONLY | **Verification package** (per re-review mode spec in `art-reviewer/SKILL.md`): Revision response checklist + residual issues list + new Decision (Accept / Minor / Major) + **R&R Traceability Matrix (Schema 11)** with Author's Claim + Verified? columns | **Narrow re-review team**: field_analyst_agent + eic_agent + editorial_synthesizer_agent (3 agents — not the full Stage 3 panel); **👁 collaboration_depth_agent (v3.5.0, advisory)** | 🧑 **Decision-heavy checkpoint:** user reviews verification decision. Hard cap: **max 1 RE-REVISE round; 2 revision loops total** across Stages 4 + 4'. Major outcome at 3' → Residual Coaching → Stage 4'. 👁 Observer runs post-checkpoint; never blocks |
| **3' → 4' Residual Coaching** | `art-reviewer` (EIC Socratic sub-stage) | VERIFIED_ONLY | Residual-issue dialogue | eic_agent | 🧑 **Decision-heavy checkpoint:** Socratic dialogue on trade-offs for residual issues (max 5 rounds). User may skip. Source: `two_stage_review_protocol.md` |
| **4'. RE-REVISE** | `art-paper` v0.1 (revision) | REDACTED | Final Revised Draft (terminal; advances to 4.5) | draft_writer_agent; revision_coach_agent; **👁 collaboration_depth_agent (v3.5.0, advisory)** | 🧑 **Decision-heavy checkpoint:** user confirms content frozen. No further review loop permitted. 👁 Observer runs post-checkpoint; never blocks |
| **4.5 FINAL INTEGRITY** | `art-pipeline` v0.1 (gate) | VERIFIED_ONLY | Updated Material Passport (`verification_status: VERIFIED`) + `repro_lock` declared — populated or explicit `null` (honest opt-out); Claim Verification Report (**final-check mode: 100% of claims** per `claim_verification_protocol.md`) | integrity_verification_agent (deeper re-run of 7 modes); state_tracker_agent. **👁 collaboration_depth_agent: SKIPPED (MANDATORY gate — observer dilution explicitly prevented)** | ✓ **Integrity gate** + user ack. **Zero-tolerance on the 7-mode re-run; no skip permitted.** Any mode SUSPECTED at 2.5 must be CLEAR or user-Overridden by 4.5. `repro_lock` is **not** read by the integrity gate at runtime (per `artifact_reproducibility_pattern.md`); if populated, `stochasticity_declaration` must be verbatim and is validated by the standalone `check_repro_lock.py` — this is post-hoc documentation, not a runtime block |
| **4→5 CLAIM-AUDIT** (v3.8, opt-in via `CRS_CLAIM_AUDIT=1`) | `art-pipeline` v0.1 (gate) | VERIFIED_ONLY | `claim_audit_results[]` + `claim_drifts[]` + `uncited_assertions[]` + `constraint_violations[]` + `audit_sampling_summaries[]` aggregates; reads `claim_intent_manifests[]` (writer-side pre-commitment baseline). Emits 5 HIGH-WARN annotation classes consumed by Stage 5 formatter REFUSE rules 6-10 | claim_ref_alignment_audit_agent (Stage 4→5 dispatch slot, after v3.7.1 cite finalizer, before formatter hard gate) | ✓ **Audit gate** (default OFF for v3.8.0). Per-citation LLM-as-judge against retrieved excerpt; 8-row finalizer matrix discriminates paywall (LOW-WARN) / fabricated (HIGH-WARN) / anchorless (HIGH-WARN) / audit_tool_failure (MED-WARN) via `ref_retrieval_method`. Calibration runner (`scripts/test_claim_audit_calibration.py`) gates with FNR<0.15 + FPR<0.10 on the shipped 20-tuple gold set. Spec: `ref/academic-research-skills/docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md` |
| **5. FINALIZE** | `art-paper` v0.1 (format-convert / disclosure) | VERIFIED_ONLY | Canonical **acmart LaTeX → PDF** (default class option `sigconf`, compiled with tectonic — IRON RULE: PDF compiled from LaTeX, never HTML-to-PDF; the `acmart` document class is obtained from CTAN or the ACM); publication-ready MD; DOCX (Pandoc, if available); AI Disclosure Statement (two-channel: AI to MAKE the artwork vs. AI to WRITE the paper) | formatter_agent | 🧑 **Decision-heavy checkpoint:** user selects format before render. Disclosure statement follows the SIGGRAPH Asia / ACM two-channel policy per `shared/references/siggraph_acm_disclosure.md` — verify exact wording, anonymization, and class option against the current SIGGRAPH Asia Art Papers CFP. **v3.8 terminal hard gate (formatter_agent REFUSE rules 6-10)** refuses output on any unresolved `[HIGH-WARN-CLAIM-NOT-SUPPORTED]` / `[HIGH-WARN-NEGATIVE-CONSTRAINT-VIOLATION]` / `[HIGH-WARN-FABRICATED-REFERENCE]` / `[HIGH-WARN-CLAIM-AUDIT-ANCHORLESS]` / `[HIGH-WARN-CONSTRAINT-VIOLATION-UNCITED]` annotation when `CRS_CLAIM_AUDIT=1` was set upstream |
| **6. PROCESS SUMMARY** | `art-pipeline` v0.1 | VERIFIED_ONLY | Paper Creation Process Record (MD + PDF); AI Self-Reflection Report (concession rate, sycophancy risk, health alerts, Failure Mode Audit Log); Score trajectory visualization; **Collaboration Depth Chapter (v3.5.0)** summarising the per-checkpoint observer reports from `collaboration_depth_history[]` | state_tracker_agent; pipeline_orchestrator_agent; **👁 collaboration_depth_agent (v3.5.0, pipeline-completion dispatch — final advisory report)** | 🧑 **Decision-heavy checkpoint:** language confirmed with user. Collaboration quality evaluated. Post-publication audit report (if peer-review published). 👁 Observer runs final pipeline-completion dispatch; never blocks |
...
flowchart LR
    ...
    Raw[art-inquiry<br/>data_access_level: raw]
    Red[art-paper<br/>data_access_level: redacted]
    Ver1[art-reviewer<br/>data_access_level: verified_only]
    Ver2[art-pipeline<br/>data_access_level: verified_only]
...
The Material Passport's `literature_corpus[]` is an **optional** Schema 9 input port for user-curated literature. Producers (out-of-band, before a art-paper session) and consumers (Phase 1 literature agents at runtime) sit on opposite sides of the passport.
...
flowchart LR
    ...
    subgraph Consumer["Phase 1 ARS runtime (v3.6.5)"]
        BA["📚 art-inquiry<br/>bibliography_agent<br/>(Phase 1)"]
        LS["📚 art-paper<br/>literature_strategist_agent<br/>(Phase 1)"]
    end
...
Authoritative references: [`art-pipeline/references/literature_corpus_consumers.md`](../art-pipeline/references/literature_corpus_consumers.md) (consumer protocol) + [`art-pipeline/references/adapters/overview.md`](../art-pipeline/references/adapters/overview.md) (adapter contract) + the ARS v3.6.5 consumer-integration design (kept as inherited provenance at [`ref/academic-research-skills/docs/design/2026-04-26-ars-v3.6.5-consumer-integration-design.md`](../ref/academic-research-skills/docs/design/2026-04-26-ars-v3.6.5-consumer-integration-design.md)).

## 6. Skill Dependency Graph

```mermaid
graph TD
    Pipeline[art-pipeline<br/>orchestrator<br/>v0.1<br/>Agent Team: 5]
    Observer[collaboration_depth_agent<br/>observer · advisory only<br/>blocking: false]
    DR[art-inquiry<br/>13 agents<br/>v0.1<br/>+ corpus reader]
    AP[art-paper<br/>12 agents<br/>v0.1<br/>+ corpus reader]
    APR[art-reviewer<br/>7 agents<br/>v0.1]
    ...
    DR -. "Concept Brief + Bibliography + Synthesis" .-> AP
...
|---|---|---|---|---|
| Concept + methodology confirmation | 🧑 | 1 | User hasn't approved Concept & Provocation Brief and Practice-Based Methodology Blueprint | Revise and re-present |
...
| VLM figure verify (v3.3) | 🤖 | 2 | Rendered documentation figure fails the figure-quality checklist (caption / credit line / image-courtesy where applicable) | Max 2 refinement iterations |
...
| Jury Chair (EIC) decision review | 🧑 | 3 | User hasn't reviewed decision letter | Present decision; await user |
...
| Disclosure check | 🤖 | 5 | Two-channel AI disclosure (artwork-making vs paper-writing) absent or wrong form per `shared/references/siggraph_acm_disclosure.md` — verify exact wording against the current CFP | Block render until fixed |
...
## 8. Inherited ARS Evolution Timeline

> This timeline records the **parent ARS suite's** release history (the genre-neutral machinery art-paper inherits). It is kept as provenance; art-paper itself starts at v0.1.0 (the art-paper fork). The version tags below are ARS-era, not art-paper-era.

```mermaid
timeline
    title ARS evolution timeline (inherited; art-paper forks at v0.1.0)
    ...
    v3.5.0 : collaboration_depth_agent (advisory observer)
           ...
           : short-stage guard + CRS_CROSS_MODEL_SAMPLE_INTERVAL
    v3.5.1 : Socratic reading-check probe (opt-in via CRS_SOCRATIC_READING_PROBE)
    ...
    v3.6.3 : Opt-in passport reset boundary (CRS_PASSPORT_RESET=1)
    ...
    v3.6.8 : Generator-Evaluator Contract Gate (v3.6.6 spec ship, naming offset)
           ...
           : two-phase orchestration in art-paper full mode (Phase 4a/4b + 6a/6b)
    ...
    v3.7.0 : Claude Code plugin packaging
           ...
           : 10 slash commands (commands/art-*.md, model pinned opus/sonnet, no haiku)
           : 3 plugin agents (agents/, symlink to v3.6.7-hardened source, model: inherit)
           : SessionStart announce hook (hooks/hooks.json + announce-art-paper-loaded.sh)
    ...
    v3.8.0 : L3 claim-faithfulness audit (second half — paired with v3.7.3)
           : claim_ref_alignment_audit_agent (opt-in via CRS_CLAIM_AUDIT=1, default OFF)
...
| Skill | Modes |
|---|---|
| `art-inquiry` v0.1 | full, quick, socratic, review, lit-review, fact-check, systematic-review (7) |
| `art-paper` v0.1 | full, plan, outline-only, revision, revision-coach, abstract-only, lit-review, format-convert, citation-check, disclosure, artist-statement, work-doc (12) |
| `art-reviewer` v0.1 | full, re-review, quick, realization-focus, guided, calibration (6) |
| `art-pipeline` v0.1 | orchestrator (delegates to sub-skill modes) + `resume_from_passport=<hash>` (v3.6.3 — resume a prior pipeline run from a Material Passport reset boundary; no flag required to invoke. The producing session must have set `CRS_PASSPORT_RESET=1` to emit boundary entries.) + `CRS_CLAIM_AUDIT=1` (v3.8 — opt-in Stage 4→5 L3 claim-faithfulness audit gate; default OFF) + v3.9.4 temporal verification advisory layer (M1 timeline_extraction_agent + M2 5-pass verifier at Phase 4→5 + M3 IRON RULE + M6 first-party Crossref/pdftotext) |
