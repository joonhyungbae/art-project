...
> 슬래시 명령: `/art-ideate` — 여러 세션에 걸쳐 주(週)·월(月) 단위로 이어지는 프로젝트.
...
파일은 플러그인이 생성 시 알려주는 경로의 로컬 Claude Code workspace에 있습니다. 기본 이름: `art-project-{slug}.md`.

매 세션, 어느 모드를 실행할지 선택합니다. 플러그인이 프로젝트 파일을 context로 로드하고, 모드를 실행하고, 세션 출력을 파일에 append합니다. 다음 세션에서는 이전 세션을 이름으로 참조할 수 있습니다 (예: "session 4 socratic에서 X라고 했는데, 여전히 holds하는가?").
...
- **단일-세션 작업에 `full`을 실행하지 마세요.** 이미 가진 재료에서 brief 하나만 작성하면 되는 경우 `/art-brief`를 직접 사용하세요.
