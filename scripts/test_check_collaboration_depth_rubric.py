...
def _make_repo(root: Path) -> None:
    ...
    agents = root / "art-pipeline" / "agents"
    ...
    (root / "art-pipeline" / "SKILL.md").write_text(_valid_skill_md_text())
...
class TestCollaborationDepthRubric(unittest.TestCase):
    ...
    def test_agent_without_rubric_ref_fails(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _make_repo(root)
            agent = root / "art-pipeline" / "agents" / "collaboration_depth_agent.md"
    ...
    def test_orchestrator_missing_invocation_fails(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _make_repo(root)
            orch = root / "art-pipeline" / "agents" / "pipeline_orchestrator_agent.md"
    ...
    def test_skill_md_without_nonblocking_phrase_fails(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _make_repo(root)
            skill = root / "art-pipeline" / "SKILL.md"
            skill.write_text("# art-pipeline\n\nWe use collaboration_depth_agent.\n")
            r = _run(root)
            self.assertEqual(r.returncode, 1)
