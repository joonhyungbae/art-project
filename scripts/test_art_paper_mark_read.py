"""Unit tests for scripts/art_paper_mark_read.py.
...
- /art-unmark-read writes rescinded_at (never deletes)
...
SCRIPT = Path(__file__).parent / "art_paper_mark_read.py"
...
# art_paper_mark_read for citation_key validation.
...
class TestMarkReadHappyPath(unittest.TestCase):
    ...
    def test_batch_form_appends_multiple_entries(self) -> None:
        """Spec §3.6: /art-mark-read accepts space-separated keys (batch form).
...
class TestMarkReadFailFast(unittest.TestCase):
    ...
    def test_no_active_passport_fails(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("[art-paper-MARK-READ ERROR:", result.stderr + result.stdout)
    ...
    def test_passport_parent_unreadable_fails(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertNotEqual(result.returncode, 0)
                self.assertIn(
                    "[art-paper-MARK-READ ERROR:", result.stderr + result.stdout
                )
                self.assertIn(
...
class TestMarkReadCitationKeyValidation(unittest.TestCase):
    def test_invalid_citation_key_hard_errors(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("[art-paper-MARK-READ ERROR:", combined)
...
class TestUnmarkRead(unittest.TestCase):
    def test_unmark_writes_rescinded_at(self) -> None:
        """Spec §3.6 firm rule 3 + Step 7: /art-unmark-read writes
    ...
    def test_unmark_unknown_key_hard_errors(self) -> None:
        """/art-unmark-read for a citation_key that was never marked is
...
class TestMarkReadYAMLPassport(unittest.TestCase):
    ...
    def test_yaml_passport_happy_path(self) -> None:
        ...
        is YAML. /art-mark-read must read it without crashing."""
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
