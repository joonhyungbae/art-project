...
dispatch automatically after `art-inquiry systematic-review → art-paper
...
1. Resolution helper (`scripts/slr_lineage.py`) — pure function over a
   ...
   produced by art-inquiry in systematic-review mode.
...
Design doc: `ref/academic-research-skills/docs/design/2026-05-15-issue-111-slr-lineage-emission-design.md`.
"""
...
class ResolveFromStagesTest(unittest.TestCase):
    ...
    the run history has `skill == 'art-inquiry'` AND
    ...
    def test_slr_mode_alias_accepted(self) -> None:
        ...
        stages = {"1": {"skill": "art-inquiry", "mode": "slr"}}
        self.assertTrue(slr_lineage.resolve_from_stages(stages))

    def test_non_deep_research_systematic_review_ignored(self) -> None:
        """A non-art-inquiry stage carrying mode='systematic-review'
        (hypothetically a future skill) must not trigger SLR lineage —
        the contract is bound to art-inquiry lineage specifically."""
        stages = {
            "1": {"skill": "art-paper", "mode": "systematic-review"},
        }
        self.assertFalse(slr_lineage.resolve_from_stages(stages))
    ...
    def test_stage_without_mode_skipped(self) -> None:
        ...
        stages = {
            "1": {"skill": "art-inquiry"},
            "4p": {"skill": "art-paper", "mode": None, "status": "skipped"},
        }
        self.assertFalse(slr_lineage.resolve_from_stages(stages))
...
class RendererIntegrationTest(unittest.TestCase):
    ...
    `art-inquiry systematic-review → art-paper full → disclosure`
    auto-dispatch path."""
    ...
    def test_non_slr_pipeline_passport_still_blocks_prisma_track(self) -> None:
        """Negative: non-SLR pipeline (e.g., art-inquiry full mode)
...
class EmitMonotonicOrTest(unittest.TestCase):
    ...
    def test_in_session_new_slr_flips_false_to_true(self) -> None:
        stages = {"1": {"skill": "art-inquiry", "mode": "systematic-review"}}
    ...
    def test_none_incoming_treated_as_false(self) -> None:
        ...
        stages = {"1": {"skill": "art-inquiry", "mode": "full"}}
