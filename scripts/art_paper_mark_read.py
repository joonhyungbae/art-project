"""art-paper /art-mark-read + /art-unmark-read CLI implementation.
...
Usage:
    python3 scripts/art_paper_mark_read.py <citation_key>... --passport-path <path>
    python3 scripts/art_paper_mark_read.py <citation_key>... --passport-path <path> --unmark

Behavior summary:
- 4 fail-fast modes (no passport / not found / parent unreadable / unwritable)
  emit canonical `[art-paper-MARK-READ ERROR: ...]` and exit non-zero.
...
- Append-only YAML write per §3.6 firm rule 3. /art-unmark-read writes
...
ERR_PREFIX = "[art-paper-MARK-READ ERROR:"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="art-paper /art-mark-read peer-file writer (v3.6.8 §3.6)."
    )
    parser.add_argument(
        "citation_keys",
        nargs="+",
