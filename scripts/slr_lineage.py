...
Pipeline plumbing helper for the `art-inquiry systematic-review →
art-paper full → disclosure --policy-anchor=prisma-trAIce` auto-
...
Design: ref/academic-research-skills/docs/design/2026-05-15-issue-111-slr-lineage-emission-design.md
...
def resolve_from_stages(stages: Mapping[str, Mapping]) -> bool:
    """Return True iff any stage was produced by art-inquiry in SLR mode.

    Run-level provenance: the contract is bound to art-inquiry lineage
    specifically, mirroring the §4.3 G2 invariant track gate. A
    non-art-inquiry stage with mode='systematic-review' does NOT
    trigger SLR lineage — only the documented producer counts.
    """
    return any(
        (stage.get("skill") == "art-inquiry")
