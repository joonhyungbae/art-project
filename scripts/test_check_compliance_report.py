...
class TestComplianceReportValidator(unittest.TestCase):
    ...
    def test_missing_maturity_warning_off_by_default(self) -> None:
        """Without CRS_WARN_MISSING_MATURITY=1, no warning when maturity is omitted.
        ...
        env.pop("CRS_WARN_MISSING_MATURITY", None)
        with TemporaryDirectory() as tmp:
            p = Path(tmp) / "r.json"
            _write(p, report)
    ...
    def test_missing_maturity_warning_opt_in(self) -> None:
        """With CRS_WARN_MISSING_MATURITY=1, warn when prisma_trAIce lacks protocol_maturity."""
        ...
        with TemporaryDirectory() as tmp:
            ...
            result = _run_with_env(
                SCRIPT, str(p),
                extra_env={"CRS_WARN_MISSING_MATURITY": "1"},
            )
        self.assertEqual(result.returncode, 0, msg=result.stderr)
