...
def fixture_repo(tmp_path: Path) -> Path:
    ...
    manifest = {
        "supported_consumers": [
            {
                "agent_path": "art-inquiry/agents/bibliography_agent.md",
    ...
    ref_dir = tmp_path / "art-pipeline" / "references"
    ...
    agent_dir = tmp_path / "art-inquiry" / "agents"
    ...
    (agent_dir / "bibliography_agent.md").write_text(
        textwrap.dedent(
            ...
            See `art-pipeline/references/literature_corpus_consumers.md`.
...
def test_l3_fails_when_backpointer_missing(fixture_repo: Path) -> None:
    agent = fixture_repo / "art-inquiry" / "agents" / "bibliography_agent.md"
    text = agent.read_text().replace(
        "art-pipeline/references/literature_corpus_consumers.md", ""
    )
...
def test_l8_passes_pr_b_with_caveat_retired_and_backpointer(fixture_repo: Path) -> None:
    ...
    manifest["supported_consumers"].append(
        {
            "agent_path": "art-paper/agents/literature_strategist_agent.md",
    ...
    ref = fixture_repo / "art-pipeline" / "references" / "literature_corpus_consumers.md"
    text = ref.read_text()
    ...
    strat_dir = fixture_repo / "art-paper" / "agents"
    strat_dir.mkdir(parents=True)
    biblio_text = (
        fixture_repo / "art-inquiry" / "agents" / "bibliography_agent.md"
    ).read_text()
...
def test_l8_fails_when_path_diverges_from_canonical(fixture_repo: Path) -> None:
    ...
    manifest = {
        "supported_consumers": [
            {
                "agent_path": "art-inquiry/agents/MOVED_bibliography_agent.md",
                "agent_basename": "bibliography_agent",
                "skill": "art-inquiry",
                "since_version": "v3.6.5",
                "phase": "Phase 1",
