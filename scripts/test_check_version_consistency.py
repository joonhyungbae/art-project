...
def _write_aligned_fixture_v351(root: Path) -> None:
    """v3.5.1 suite: art-inquiry 2.9.1, art-pipeline 3.5.1."""
    skills = [
        ("art-inquiry", "2.9.1"),
        ("art-paper", "3.1.0"),
        ("art-reviewer", "1.8.1"),
...
class TestVersionConsistency(unittest.TestCase):
    ...
    def test_all_aligned_v351_passes(self) -> None:
        """v3.5.1 suite (art-inquiry 2.9.1, art-pipeline 3.5.1) must pass."""
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
    ...
    def test_table_version_drift_fails(self) -> None:
        """Table lists art-inquiry v2.9.0 but SKILL.md says 2.8.0 — must fail."""
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_aligned_fixture(root)
            _write_skill(root, "art-inquiry", "2.8.0")
            result = _run(root)
            self.assertEqual(result.returncode, 1)
            self.assertIn("art-inquiry", result.stdout)
    ...
    def test_pipeline_version_vs_suite_drift_fails(self) -> None:
        """art-pipeline version in table must equal suite version."""
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_aligned_fixture(root)
            ...
            # (SKILL.md == table).
            _write_skill(root, "art-pipeline", "3.4.0")
            _write_claude_md(
                root,
                suite_version="3.5.0",
                table_rows=[
            ...
            result = _run(root)
            self.assertEqual(result.returncode, 1)
            self.assertIn("art-pipeline", result.stdout)

    def test_skill_missing_frontmatter_fails(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_aligned_fixture(root)
            (root / "art-inquiry" / "SKILL.md").write_text(
                "# art-inquiry\n\nNo frontmatter here.\n",
                encoding="utf-8",
            )
            result = _run(root)
    ...
    def test_skill_listed_in_table_but_missing_on_disk_fails(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            _write_aligned_fixture(root)
            import shutil
            shutil.rmtree(root / "art-reviewer")
            result = _run(root)
            self.assertEqual(result.returncode, 1)
            self.assertIn("art-reviewer", result.stdout)
        ...
        with TemporaryDirectory() as tmp:
            ...
            (root / ".claude" / "CLAUDE.md").write_text(
                textwrap.dedent(
                    ...
                    | Skill | Purpose | Key Modes |
                    |-------|---------|-----------|
                    | `art-inquiry` v2.9.0 | x | y |
                    | `art-paper` v3.1.0 | x | y |
                    | `art-reviewer` v1.8.1 | x | y |
                    | `art-pipeline` v3.5.0 | x | y |
    ...
    def test_invalid_table_row_token_is_reported(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            table_rows_with_junk = [
                ...
                ("art-paper", "3.1.2"),
                ("art-reviewer", "1.9.1"),
                ("art-pipeline", "3.9.4.2-alpha"),  # invalid token
            ]
            _write_claude_md(
    ...
    def test_four_segment_table_row_drift_fails(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            table_rows_drifted = [
                ...
                ("art-paper", "3.1.2"),
                ("art-reviewer", "1.9.1"),
                ("art-pipeline", "3.9.4.1"),  # drift inside the 4th segment
            ]
            _write_claude_md(
