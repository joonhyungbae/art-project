...
def warn_suspicious(report: dict) -> list[str]:
    ...
    # Opt-in (CRS_WARN_MISSING_MATURITY=1) to preserve the zero-touch
    # promise for entries authored before Schema 12's protocol_maturity
    # field landed.
    if os.environ.get("CRS_WARN_MISSING_MATURITY") == "1":
        if isinstance(pt, dict) and "protocol_maturity" not in pt:
            warnings.append(
                ...
                "(CRS_WARN_MISSING_MATURITY=1) and does not fail validation."
            )
    return warnings
