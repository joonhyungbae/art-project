...
`CRS_PASSPORT_RESET` must co-locate a reference to the protocol-doc stem
`passport_as_reset_boundary` so readers can follow the flag to the spec.

The protocol doc itself (`art-pipeline/references/passport_as_reset_boundary.md`)
...
class TestPassportResetContractLint(unittest.TestCase):
    ...
    def test_mention_without_protocol_ref_fails(self):
        with TemporaryDirectory() as td:
            root = Path(td)
            _write(root, "some_doc.md", "Set CRS_PASSPORT_RESET=1 to enable the flag.\n")
            result = run_script(SCRIPT, "--root", str(root))
    ...
    def test_mention_with_protocol_ref_passes(self):
        with TemporaryDirectory() as td:
            ...
            _write(
                root,
                "some_doc.md",
                "Set CRS_PASSPORT_RESET=1 to enable.\n"
                "See art-pipeline/references/passport_as_reset_boundary.md for details.\n",
            )
            result = run_script(SCRIPT, "--root", str(root))
    ...
    def test_protocol_doc_itself_passes_without_self_reference(self):
        with TemporaryDirectory() as td:
            ...
            _write(
                ...
                "This protocol is activated by the CRS_PASSPORT_RESET env flag.\n",
            )
            result = run_script(SCRIPT, "--root", str(root))
            self.assertEqual(result.returncode, 0, msg=result.stdout + result.stderr)
    ...
    def test_json_description_satisfies_reference(self):
        with TemporaryDirectory() as td:
            ...
            _write(
                ...
                '{"description": "Set CRS_PASSPORT_RESET=1; see '
                'art-pipeline/references/passport_as_reset_boundary.md for protocol."}\n',
            )
            result = run_script(SCRIPT, "--root", str(root))
            self.assertEqual(result.returncode, 0, msg=result.stdout + result.stderr)
    ...
    def test_code_comment_still_requires_reference(self):
        with TemporaryDirectory() as td:
            root = Path(td)
            _write(root, "scripts/foo.py", "# CRS_PASSPORT_RESET handling\n")
            result = run_script(SCRIPT, "--root", str(root))
            self.assertEqual(result.returncode, 1, msg=result.stdout + result.stderr)
    ...
    def test_binary_file_skipped(self):
        with TemporaryDirectory() as td:
            root = Path(td)
            blob = root / "blob.bin"
            blob.write_bytes(b"\x00\x01\x02CRS_PASSPORT_RESET\x03\x04\xff\xfe")
            result = run_script(SCRIPT, "--root", str(root))
